...
nPointsError=11;
errorVec01=zeros(nPointsError,3);
...
for k = 3:nPointsError+2
    ...
    D1x(nx,nx-1:nx)=[4,1];
    D2x(nx,nx-4:nx)=[-1/(12*delta_x) 2/(3*delta_x) -3/delta_x -2/(3*delta_x) 37/(12*delta_x)];
    %Para el punto extremo "i=end-1"
    D1x(nx-1,nx-2:nx-1)=[4,1];
    D2x(nx-1,nx-5:nx-1)=[-1/(12*delta_x) 2/(3*delta_x) -3/delta_x -2/(3*delta_x) 37/(12*delta_x)];
    %Para el punto extremo "i=1"
    D1x(1,1:2)=[1 4];
    D2x(1,1:5)=[-37/(12*delta_x) 2/(3*delta_x) 3/delta_x -2/(3*delta_x) 1/(12*delta_x)];
    %Para el punto extremo "i=2"
    D1x(2,2:3)=[1 4];
    D2x(2,2:6)=[-37/(12*delta_x) 2/(3*delta_x) 3/delta_x -2/(3*delta_x) 1/(12*delta_x)];
    ...
    errorVec01(k-2,1:3)=[nx delta_x max(error)];
end

%figure; hold on;
%loglog(errorVec01(:,1),errorVec01(:,2),'ko-'); grid on;
